...
public:
    ...
    T value(std::size_t idx = 0) const;
    ...
    T value_or(const T& default_value, std::size_t idx = 0) const;
...
template <class T>
inline std::size_t Value<T>::count() const
{
    return values_.size();
}
...
template <class T>
inline T Value<T>::value_or(const T& default_value, std::size_t idx) const
{
    if (idx < values_.size())
        return values_[idx];
    else if (default_)
...
template <class T>
inline T Value<T>::value(std::size_t idx) const
{
